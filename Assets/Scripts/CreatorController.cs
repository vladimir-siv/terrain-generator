...
public class CreatorController : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			var rotSpeed = RotationalSpeed * Time.deltaTime;
		...
		transform.position += (transform.forward * direct + transform.right * strafe + transform.up * fly).normalized * Speed * Time.deltaTime;
	}
}
